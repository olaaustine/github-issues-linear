...
from errors import ResponseNot200Error, GraphQLError
...
def test_response_status_check_http_error():
    ...
    with pytest.raises(ResponseNot200Error):
        linear_service.response_status_check(response)
...
def test_get_issues_if_it_exists_true(mock_get_team_id, mock_post):
    ...
    with patch("linear_service.response_status_check") as mock_check:
        mock_check.return_value = None
        assert (
            linear_service.get_issues_if_it_exists("title", "url", {}, "yathqqqqq")
            is True
        )
